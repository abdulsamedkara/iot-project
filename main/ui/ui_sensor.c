...
void ui_sensor_update(float temp_c, int ldr_value, float noise_db) {
    ...
    // ─── Işık Güncelle ────────────────────────────────────────
    int light_pct = ldr_value; // main.c zaten 0-100 arasi percentage olarak gönderiyor
    ...
    // ─── Konfor Skoru (Sıcaklık + Işık + Gürültü ortalaması) ──────────────────
    int noise_score;
    if (noise_pct < 30)       noise_score = 100; // sessiz
    else if (noise_pct < 55)  noise_score = 60;  // normal
    else                       noise_score = 20;  // gürültülü
