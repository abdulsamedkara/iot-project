...
void ui_voice_init(lv_obj_t *parent) {
    ...
    lv_label_set_text(lbl_title, STR_VOICE_TITLE);
    ...
    lv_label_set_text(lbl_status, STR_WAITING);
    lv_obj_align(lbl_status, LV_ALIGN_TOP_MID, 0, 120);

    // ─── Transcript Area ─────────────────────────────────────
    // Taller box, enough space for "Heard:" + text
    ...
    lv_label_set_text(lbl_heard, STR_HEARD);
    lv_obj_align(lbl_heard, LV_ALIGN_TOP_LEFT, 0, 0);

    // Transcript text — pure white, scrolling
...
void ui_voice_set_state(voice_ui_state_t state) {
    if (state == VOICE_RECORDING) {
        ...
        lv_label_set_text(lbl_status, STR_LISTENING);
        ...
        lv_label_set_text(lbl_status, STR_PROCESSING);
