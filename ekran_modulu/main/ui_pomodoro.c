...
void ui_pomodoro_init(lv_obj_t *parent) {
    ...
    lv_label_set_text(lbl_mode, "HAZIR");
...
void ui_pomodoro_update(pomo_state_t state, uint32_t remaining_sec, uint32_t total_sec, int completed_count) {
    ...
    if (state == POMO_WORK) {
        lv_label_set_text(lbl_mode, "ODAKLAN");
        mode_color = lv_color_hex(0x4CAF50); // Yeşil
        lv_obj_set_style_arc_color(arc_time, mode_color, LV_PART_INDICATOR);
    } else if (state == POMO_BREAK) {
        lv_label_set_text(lbl_mode, "DINLEN");
        mode_color = lv_color_hex(0x29B6F6); // Mavi
        lv_obj_set_style_arc_color(arc_time, mode_color, LV_PART_INDICATOR);
    } else {
        lv_label_set_text(lbl_mode, "HAZIR");
        mode_color = lv_color_hex(0x607d8b); // Gri
        lv_obj_set_style_arc_color(arc_time, mode_color, LV_PART_INDICATOR);
    }
