...
void ui_sensor_init(lv_obj_t *parent) {
    ...
    lv_label_set_text(lbl_title, "ORTAM ANALIZI");
    lv_obj_align(lbl_title, LV_ALIGN_TOP_MID, 0, 8);

    // ─── Uyarı İkonu (başlangıçta gizli) ─────────────────────
    ...
    // =========================================================
    // KART 1 — SICAKLIK (Ekranın sol yarısı)
    // =========================================================
    ...
    // Kart başlığı satırı: ikon + etiket
    ...
    lv_label_set_text(lbl_temp_title, "SICAKLIK");
    lv_obj_align_to(lbl_temp_title, lbl_temp_icon, LV_ALIGN_OUT_RIGHT_MID, 3, 0);

    // Sıcaklık değeri — büyük, belirgin
    ...
    // =========================================================
    // KART 2 — IŞIK YOĞUNLUĞU (Sağ yarı)
    // =========================================================
    ...
    lv_label_set_text(lbl_light_title, "ISIK");
    ...
    lv_label_set_text(lbl_light_state, "IYI");
    lv_obj_align(lbl_light_state, LV_ALIGN_BOTTOM_MID, 0, 0);

    // =========================================================
    // ALT KART — KONFOR SKORU
    // =========================================================
    ...
    lv_label_set_text(lbl_comfort_title, "KONFOR");
...
void ui_sensor_update(float temp_c, int ldr_value) {
    ...
    int temp_score;
    if (temp_c < 18.0) {
        temp_color = lv_color_hex(0x29B6F6); temp_str = "SOGUK"; temp_score = 40;
    } else if (temp_c > 28.0) {
        temp_color = lv_color_hex(0xFF5252); temp_str = "SICAK"; temp_score = 30;
    } else {
        temp_color = lv_color_hex(0x4CAF50); temp_str = "KONFOR"; temp_score = 100;
    }
    lv_obj_set_style_text_color(lbl_temp_state, temp_color, 0);
    lv_label_set_text(lbl_temp_state, temp_str);

    // ─── Işık Güncelle ────────────────────────────────────────
    int light_pct = (ldr_value * 100) / 4095;
    ...
    if (light_pct < 15) {
        light_str = "KARANLIK"; light_color = lv_color_hex(0xFF5252); light_score = 20;
    } else if (light_pct < 30) {
        light_str = "ZAYIF"; light_color = lv_color_hex(0xFFA726); light_score = 60;
    } else {
        light_str = "IYI"; light_color = lv_color_hex(0x4CAF50); light_score = 100;
    }
    lv_obj_set_style_text_color(lbl_light_state, light_color, 0);
    lv_label_set_text(lbl_light_state, light_str);

    // ─── Konfor Skoru ─────────────────────────────────────────
    int comfort = (temp_score + light_score) / 2;
